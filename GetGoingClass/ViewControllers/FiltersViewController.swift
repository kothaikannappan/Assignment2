...
class FiltersViewController: UIViewController {
    ...
    @IBAction func reset(_ sender: UIButton) {
        FilterService.shared.delegate?.applyFilter(rankBy: nil, rad: nil, open: nil )
    ...
    @IBAction func radiusSliderChangedValue(_ sender: UISlider) {
        ...
        rad = Double(exactly: sender.value)

    }
    @IBAction func saveFilters(_ sender: UIBarButtonItem) {
        FilterService.shared.delegate?.applyFilter(rankBy: rankBySelectedLabel.text, rad: rad, open: open )
        dismiss(animated: true, completion: nil)
    }
...
extension FiltersViewController: UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        rankBySelectedLabel.text = rankByDictionary[row].description().lowercased()
    }
}
